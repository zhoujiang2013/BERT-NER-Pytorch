...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = -1 * model._modules['module'].loss_fn(emissions = logits, tags=inputs['labels'], mask=inputs['attention_mask'])
            if args.n_gpu > 1:
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            tmp_eval_loss = -1 * model._modules['module'].loss_fn(emissions = logits, tags=inputs['labels'], mask=inputs['attention_mask'])
        if args.n_gpu > 1:
